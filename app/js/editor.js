	...
	var methods = {
		...
		imageWidget: function(){
			...
			handleFileSelect = function(evt) {
				...
				for (var i = 0, f; f = files[i]; i++) {
					...
					if(!f.type.match('image.*') || !f.name.match(/(?:gif|jpg|png|jpeg)$/)){ //Process only Images
						methods.showMessage.apply(this,["imgErrMsg","Type de fichier invalide"]);
						continue;
					}
			...
			var getURL = $("<button/>",{
				...
				if(url ==''){
					methods.showMessage.apply(this,["imgErrMsg","Entrer l'url de l'image"]);
					return false;
				}
				...
				var image = $('<img/>',{
					src:url,
				  	...
				  	methods.showMessage.apply(this,["imgErrMsg","Url invalide"]);
				  	return false;
				...
				class:"row-fluid"
					...
					id:"tblForm"+idExtn 
						...
						class:"row" 
							...
							class:"col-xs-12 col-sm-6 col-md-6 col-lg-6"
							}).append($('<label/>',{ for:"tblAlign"+idExtn, text:"Aligement"}
								...
								)).append($('<option/>',{ text:"Centrer", value:"center"}
		...
		init : function( options )
		{
			...
			var menuItems = { 'fonteffects': true,
						     ...
						     'ol'		: { "text": "N", 
											"icon": "fa fa-list-ol", 
											"tooltip": "Inserer/enlever une liste ordonnée",
											"commandname":"insertorderedlist", 
											"custom":null },

						     'ul'		: { "text": "Bullet", 
											"icon": "fa fa-list-ul", 
											"tooltip": "Inserer/enlever une liste non ordonnée",
						   ...
						   'insert_link': { "modal": true,
						   					"modalId": "InsertLink", 
											"icon":"fa fa-link", 
											"tooltip": "Inserer un lien",
											"modalHeader": "Insertion de lien",
											...
											"onSave":function(){
												...
												if(targetURL ==''){
													methods.showMessage.apply(editorObj,["errMsg","Entrer l'url"]);
													return false;
												}												
												if(!targetURL.match(urlPattern)){
													methods.showMessage.apply(editorObj,["errMsg","Entrer une url valide"]);
													return false;
												}													
												if(range=='' && targetText==''){ 
						   ...
						   'insert_img'	: { "modal": true,
											...
											"tooltip": "Inserer une image",
											"modalHeader": "Insertion d'Image",
											...
											"onSave": function(){
												...
												if($('#imageList').data('current')){
													...
													methods.showMessage.apply(this,["imgErrMsg","Selectionner une image"]);
													return false;
												}
						...
						'insert_table'	: { "modal": true,
					   						"modalId": "InsertTable", 
											"icon":"fa fa-table", 
											"tooltip": "Inserre un tableau",
											"modalHeader": "Insertion de tableau",
											...
											"onSave": function(){
												...
												if(!tblRows.match(intReg)){
													methods.showMessage.apply(this,["tblErrMsg","Le nombre de ligne doit être positif"]);
													return false;
												}													
												if(!tblColumns.match(intReg)){
													methods.showMessage.apply(this,["tblErrMsg","Le nombre de colonne doit être positif"]);
													return false;
												}
												if(tblWidth!="" && !tblWidth.match(cssReg)){
													methods.showMessage.apply(this,["tblErrMsg","Entrer un nombre valide avec les unités de mésure CSS (px,em,ex,%,in,cm,mm,pt,pc)"]);
													return false;
												}
												...
												if(tblBorder!="" && !tblBorder.match(numReg)){
													methods.showMessage.apply(this,["tblErrMsg","Taille de la bordure doit être positive"]);
													return false;
												}
												if(tblCellspacing!="" && !tblCellspacing.match(numReg)){
													methods.showMessage.apply(this,["tblErrMsg","La taille des cellule doit être positive"]);
													return false;
												}
												if(tblCellpadding!="" && !tblCellpadding.match(numReg)){
													methods.showMessage.apply(this,["tblErrMsg","La marge doit être positive"]);
													return false;
												}
						   ...
						   'outdent'	: { "text": "Outdent", 
							...
							'rm_format'	: { "text": "Remove format", 
											"icon":"fa fa-eraser", 
											"tooltip": "Enlever la mise en page",
							...
							'splchars'	: { "text": "S", 
											"icon": "fa fa-asterisk", 
											"tooltip": "Inserer un caractère speciale",
											"commandname":null, 
											"custom":function(button){
